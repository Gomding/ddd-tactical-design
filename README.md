...
- **주문 (Order)**
  ...
  - [ ] 주문 유형은 다음과 같다.
    ...
    - 포장 - TAKE_OUT
    - 매장 식사 - EAT_IN
  - [x] 주문의 유형은 필수 값으로 배달, 테이크아웃, 매장식사 중 하나를 선택해야한다.
  - [ ] 주문 상태는 다음과 같다.
    - 대기 - WAITING
    - 수락 - ACCEPTED
    - 제공 - SERVED
    ...
    - 배달완료 - DELIVERED
    - 주문 완료 - COMPLETED
  - [x] 주문 메뉴는 하나 이상의 정보가 존재해야 한다.
    - [x] 주문에 등록할 메뉴는 존재하는 메뉴여야 한다.
      - [x] 존재하지 않는 메뉴라면 주문이 불가능함을 사용자에게 알려야한다.
    - [x] 주문에 등록된 메뉴는 고객에게 전시중인 메뉴여야 한다.
      - [x] 전시중인 메뉴가 아니면 주문이 불가능하다.
    - [x] 주문유형이 매장식사(EAT_IN)가 아닐 때 주문 메뉴의 메뉴 개수는 0 미만의 음수가 될 수 없다.
    ...
    - [x] 주문 메뉴는 실제 메뉴와 가격이 동일해야 한다.
      - [x] 주문 메뉴는 실제 메뉴와 가격이 동일하지 않으면 사용자에게 주문 불가능함을 알려야한다.
  - [x] 새로운 주문을 등록할 수 있다.
    - [x] 주문을 새로 등록하면 주문 상태는 대기(WATING) 상태이다.
    - [x] 배달 주문(DELIVERY)일 때 주문 테이블 정보는 가질 필요가 없다.
    - [x] 배달 주문은 배달 주소가 필수 값이다.
      - [x] 배달 주문 시 배달 주소값이 비어있으면 사용자에게 주문할 수 없음을 알려야한다.
    - [x] 매장 식사(EAT_IN) 주문은 주문 테이블 정보가 필수이다.
      - [X] 매장 식사 주문에 등록하려는 주문 테이블은 미리 등록된 주문 테이블이어야 한다.
        - [x] 주문 테이블이 존재하지 않으면 사용자에게 매장 식사 주문이 불가능함을 알려야한다.
      - [x] 매장 식사 주문은 사용중인 주문 테이블만 주문 등록이 가능하다.
      - [x] 매장 식사 주문은 배달 주소 값을 가질 필요가 없다.
    - [x] 포장(TAKE_OUT) 주문은 배달 주소, 주문 테이블 정보 둘 다 필요하지 않다.
  - [x] 주문의 상태를 수락(ACCEPT)상태로 변경할 수 있다.
    - [x] 변경하고자 하는 주문은 존재하는 주문이어야한다.
      - [x] 존재하지 않는 주문이라면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 주문 상태가 대기(WAITING)이 아니라면 수락 상태로 변경할 수 없다.
    - [x] 배달 주문이라면 (주문고유 번호, 주문의 가격, 배달 주소) 정보를 배달기사 배정 회사에 보내서 배달을 요청해야한다.
  - [x] 주문을 제공(SERVED) 상태로 변경할 수 있다.
    - [x] 변경하고자 하는 주문은 존재하는 주문이어야한다.
      - [x] 존재하지 않는 주문이라면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 주문 상태가 수락(ACCEPT)상태가 아니라면 제공 상태로 변경할 수 없다.
  - [x] 주문을 배달 중(DELIVERING) 상태로 변경할 수 있다.
    - [x] 배달 중 상태로 변경 하려는 주문은 존재하는 주문이어야한다.
      - [x] 존재하지 않는 주문이라면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 배달 주문일 때만 변경이 가능하다.
      - [x] 포장(TAKE_OUT), 매장 식사(EAT_IN)라면 배달 중 상태로 변경이 불가능하다.
    - [x] 주문이 제공상태가 아니라면 변경이 불가능하다.
  - [x] 배달 완료(DELIVERED) 상태로 변경이 가능하다.
    - [x] 배달완료 상태로 변경 하려는 주문은 존재하는 주문이어야한다.
      - [x] 존재하지 않는 주문이라면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 주문 상태가 배달중 상태가 아니라면 변경이 불가능하다.
  - [x] 주문 완료(COMPLETE) 상태로 변경이 가능하다.
    - [x] 주문 완료 상태로 변경 하려는 주문은 존재하는 주문이어야한다.
      - [x] 존재하지 않는 주문이라면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 배달 주문일 때 배달 완료(DELIVERED)가 아니라면 주문 완료로 변경이 불가능하다.
    - [x] 포장 또는 매장 식사 일 때 제공(SERVED)상태가 아니라면 주문 완료로 변경이 불가능하다.
    - [x] 주문 완료 변경 후 매장 식사 주문이라면 주문 테이블을 비어있는 테이블로 변경해야한다.
  - [ ] 전체 주문정보를 조회할 수 있다.
...
- **주문 테이블 (Order Table)**
  ...
  - [x] 주문 테이블에 고객이 앉았음을 등록할 수 있다.
    - [x] 대상 주문 테이블은 존재하는 주문 테이블이어야 한다.
      - [x] 존재하지 않는 주문 테이블이면 사용자에게 잘못된 상황임을 알려야한다.
  - [x] 고객이 앉았던 주문 테이블 정보를 비어있는 테이블로 변경할 수 있다.
    - [x] 주문 테이블을 비어있는 테이블로 변경할 때 주문 테이블은 존재해야한다.
      - [x] 존재하지 않는 주문 테이블이면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 주문 테이블의 (주문이 존재하고) -> (주문 상태가 `주문 완료(COMPLETED)`가 아니라면) 비어있는 테이블로 변경할 수 없다.
    - [x] 비어있는 테이블이란 (고객수 = 0, 테이블 미사용) 상태를 의미한다.
  - [x] 주문 테이블에 앉은 고객 수를 변경할 수 있다.
    - [x] 수정할 때 고객 수는 0이상의 양수여야 한다.
      - [x] 고객 수를 0미만의 음수로 등록하면 사용자에게 잘못된 등록임을 알려야한다.
    - [x] 주문 테이블의 고객 수를 변경할 때 주문 테이블은 존재해야한다.
      - [x] 존재하지 않는 주문 테이블이면 사용자에게 잘못된 상황임을 알려야한다.
    - [x] 주문 테이블이 사용 중인 상태일 때만 고객 수를 변경할 수 있다.
  - [x] 전체 주문 테이블의 정보를 조회할 수 있다.
    - [x] 각 주문 테이블의 (주문테이블 고유번호, 이름, 테이블에 앉은 고객 수, 사용 여부)를 조회할 수 있어야한다.
